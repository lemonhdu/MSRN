...
def get_time_points(predicted):
    ...
    for i in range(predicted_length):
        ...
        if predicted[0][i] == 1 and predicted[0][i+1] == 2 and state_index == 'idle':
            points.append(i+1)
            state_index = 'drop'
...
def get_stage_features(i3d_feature, tcn_predict_time_points, time_point_label_length):

    i3d_feature = i3d_feature.squeeze(0).transpose(1, 0)

    if time_point_label_length == 3:
        # [jumping, dropping point]
        i3d_feature_jumping = i3d_feature[:tcn_predict_time_points[0][0]]

    elif time_point_label_length == 4:
